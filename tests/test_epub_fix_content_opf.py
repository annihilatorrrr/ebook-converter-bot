...
class TestFixContentOpfProblems(unittest.TestCase):
    def test_dedup_sort_and_case_fix(self) -> None:
...
"""
            ...
            assert [c.get("id") for c in items if c.get("id", "").startswith("page_")] == ["page_1", "page_2", "page_3"]
    ...
    def test_fix_content_opf_dedups_zip_and_stores_mimetype(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            epub_path = Path(tmp_dir) / "book.epub"
...
"""
    ...
    def test_set_epub_to_rtl_dedups_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            epub_path = Path(tmp_dir) / "book.epub"
...
"""
    ...
    def test_flatten_toc_dedups_and_is_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            epub_path = Path(tmp_dir) / "book.epub"
